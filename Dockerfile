...
COPY extras/confd/conf.d/* /etc/confd/conf.d/
COPY extras/confd/templates/* /etc/confd/templates/
...
RUN sed -i 's/env python/&3/' /patroni*.py \
    ...
    && chmod +s /bin/ping \
    && chown -R postgres:postgres "$PGHOME" /run /etc/haproxy /etc/pgbouncer /etc/confd /etc
